...
# Usage: curl -sfL https://raw.githubusercontent.com/${GITHUB_ORG:-antonioacg}/infra-management/${GIT_REF:-main}/bootstrap.sh | GITHUB_TOKEN="ghp_xxx" bash -s -- --nodes=1 --tier=small [--start-phase=N]
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            log_info "Usage: --nodes=N --tier=SIZE [--start-phase=N]"
...
if [[ -z "$GITHUB_TOKEN" ]]; then
    ...
    log_info ""
    log_info "Usage:"
    log_info "  curl -sfL https://raw.githubusercontent.com/${GITHUB_ORG}/infra-management/${GIT_REF}/bootstrap.sh | GITHUB_TOKEN=\"ghp_xxx\" bash -s -- --nodes=N --tier=SIZE"
    log_info ""
...
print_banner "🚀 Enterprise Platform Bootstrap" "Ultra-streamlined orchestrator" "Resources: $NODE_COUNT nodes, $RESOURCE_TIER tier, start: Phase $START_PHASE"
...
# Phase 2+: Infrastructure (when implemented)
