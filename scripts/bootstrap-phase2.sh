...
_validate_phase2_prerequisites() {
    ...
    if [[ -z "${TF_VAR_minio_root_user:-}" || -z "${TF_VAR_minio_root_password:-}" || -z "${TF_VAR_postgres_password:-}" || -z "${ENCRYPTION_PASSPHRASE:-}" ]]; then
        ...
        log_info "  - ENCRYPTION_PASSPHRASE (from Phase 1)"
        log_info ""
...
EOF
        ...
        tofu init -backend-config=backend-remote.hcl
    fi
}
