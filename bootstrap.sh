...
if [[ ! "$START_PHASE" =~ ^[01]$ ]]; then
    log_error "Start phase must be 0 or 1 (got: '$START_PHASE')"
    log_info "Phase 2 requires credentials generated by Phase 1 (in-memory only)"
    exit 1
fi

if [[ -n "$STOP_AFTER" ]] && [[ ! "$STOP_AFTER" =~ ^(0|1|2[a-d])$ ]]; then
    log_error "Stop after must be 0, 1, 2a, 2b, 2c, or 2d (got: '$STOP_AFTER')"
    exit 1
fi
